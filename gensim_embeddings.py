...
def build_keras_model_sg(index_size, vector_size,
    ...
    kerasmodel.add_node(Embedding(index_size, vector_size, input_length=sub_batch_size, weights=[model.syn0]),
                        name='embedding', input='index')
    kerasmodel.add_node(Embedding(context_size, vector_size, input_length=sub_batch_size, weights=[model.keras_syn1]),
                        name='embedpoint', input='point')
...
def build_keras_model_cbow(index_size, vector_size,
    ...
    kerasmodel.add_node(Activation('sigmoid'), name='sigmoid', inputs=['average', 'embedpoint'], merge_mode='dot',
                        dot_axes=-1)
    kerasmodel.add_output(name='code', input='sigmoid')
    kerasmodel.compile('rmsprop', {'code': 'mse'})
    return kerasmodel
...
def train_prepossess(model):
    ...
    if model.hs:
        model.word_context_size_max += max(
            len(model.vocab[w].point) for w in model.vocab if hasattr(model.vocab[w], 'point'))
    if model.negative > 0:
...
if __name__ == "__main__":
    ...
    print
    'compare', brk.compare_w2v(br)
    brk.train(brown_sents)
    print
    'compare', brk.compare_w2v(br)
    print
    brk.most_similar_cosmul(positive=['she', 'him'], negative=['he'], topn=topn)

    br_dummy = gensim.models.word2vec.Word2Vec(brown_sents, sg=sg_v, iter=1)
    copy_word2vec_instance_from_to(brk, br_dummy)
    print
    br_dummy.most_similar_cosmul(positive=['she', 'him'], negative=['he'], topn=topn)
    print(br_dummy.most_similar('the', topn=5))

    print
    br.most_similar_cosmul(positive=['she', 'him'], negative=['he'], topn=topn)
    print
    brk.most_similar_cosmul(positive=['she', 'him'], negative=['he'], topn=topn)
